...
export async function interpreterSubmitInvoice(db, user, payload) {
  ...
  const link = await db.from("assignment_interpreters").select("id,assignment_id,status,assignments(service_type,start_at)").eq("assignment_id", payload.assignmentId).eq("interpreter_id", interpreter.id).maybeSingle();
  ...
  const result = await db.from("contractor_invoices").insert({
    ...
    invoice_date: payload.invoiceDate || now.slice(0, 10),
    amount,
...
export async function adminReviewContractorInvoice(db, user, payload) {
  ...
  const updates = {
    ...
    admin_notes: String(payload.notes || "").trim() || null,
    reviewed_by: user.id,
    reviewed_at: now,
