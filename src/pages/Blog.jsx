...
function BlogBrandVisual({ palette }) {
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-[2.4rem] border bg-white/90 p-5 shadow-2xl backdrop-blur" style={{ borderColor: palette.border }}>
        <div className="absolute inset-0 opacity-80" style={{ background: "radial-gradient(circle at 15% 15%, rgba(221,125,0,0.18), transparent 26%), radial-gradient(circle at 92% 10%, rgba(114,17,0,0.14), transparent 28%), linear-gradient(135deg, #ffffff 0%, #f8f3ef 100%)" }} />

        <div className="relative rounded-[1.9rem] border bg-white p-5 shadow-sm" style={{ borderColor: palette.border }}>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={item.label} className="rounded-2xl border bg-[#faf8f6] p-4" style={{ borderColor: palette.border }}>
                <p className="text-[10px] font-black uppercase tracking-[0.18em]" style={{ color: palette.burgundy }}>{item.label}</p>
          ...
          <div className="mt-5 overflow-hidden rounded-[1.5rem] border" style={{ borderColor: palette.border }}>
            <div className="flex items-center gap-3 bg-[#202020] px-4 py-3 text-white">
              ...
              <div>
                ...
                <p className="text-xs text-white/65">Practical, ethical, consumer-centered guidance.</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-px bg-[#e7ddd3]">
              {[
                { Icon: ShieldCheck, text: "Ethics" },
                { Icon: BookOpen, text: "Prep" },
                { Icon: Sparkles, text: "Access" },
              ].map(({ Icon, text }) => (
                <div key={text} className="flex flex-col items-center gap-2 bg-white px-3 py-4 text-center text-xs font-black" style={{ color: palette.charcoal }}>
                  <Icon size={18} style={{ color: palette.burgundy }} />
                  {text}
                </div>
              ))}
            </div>
...
function PostMeta({ post, palette, light = false }) {
  return (
    <div className={`flex flex-wrap gap-3 text-sm ${light ? "text-white/75" : "text-[#777]"}`}>
    ...
    <div className="overflow-hidden bg-white">
      <section className="relative px-5 py-14 md:px-8 md:py-20">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          <motion.div initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.55 }}>
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border bg-white/85 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] shadow-sm backdrop-blur" style={{ borderColor: palette.border, color: palette.burgundy }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {[
                ...
                <span key={item} className="rounded-full border bg-white/80 px-4 py-2 text-sm font-bold shadow-sm" style={{ borderColor: palette.border, color: palette.charcoal }}>
                  {item}
                </span>
              ))}
            </div>
      ...
      <section className="px-5 py-12 md:px-8 md:py-16">
        <div className="mx-auto max-w-6xl">
          {featuredPost ? (
            <motion.article initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} variants={fadeUp} transition={{ duration: 0.45 }} className="overflow-hidden rounded-[2.2rem] border bg-white shadow-2xl" style={{ borderColor: palette.border }}>
              <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
                ...
                <div className="flex flex-col justify-center p-7 md:p-10">
                  ...
                  <div className="mt-7 rounded-2xl border bg-[#faf8f6] p-5" style={{ borderColor: palette.border }}>
            ...
            <div className="rounded-[2rem] border bg-[#fafafa] p-8 text-center" style={{ borderColor: palette.border }}>
              ...
              <p className="mt-3 text-[#666]">Approved scheduled posts will appear here automatically when their publish date arrives.</p>
            </div>
          )}
            ...
            <div className="mt-14">
              ...
              <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.12 }} variants={staggerGroup} className="mt-8 grid auto-rows-fr gap-6 md:grid-cols-2 lg:grid-cols-3">
                {remainingPosts.map((post) => {
                  ...
                  return (
                    <motion.article key={post.slug} variants={fadeUp} transition={{ duration: 0.45 }} className="group flex overflow-hidden rounded-[1.8rem] border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl" style={{ borderColor: palette.border }}>
                      <Link to={`/blog/${post.slug}`} className="flex w-full flex-col focus:outline-none focus:ring-4" style={{ "--tw-ring-color": "rgba(221, 125, 0, 0.35)" }}>
                        <div className="relative h-48 overflow-hidden bg-[#f7f3ef]">
                          {postImage ? (
                            ...
                            <div className="flex h-full items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(221,125,0,0.20), rgba(114,17,0,0.16))" }}>
                              <FileText size={38} style={{ color: palette.burgundy }} />
                            </div>
                          )}
                          <div className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] shadow-sm backdrop-blur" style={{ color: palette.burgundy }}>
                            {post.category}
                          </div>
                        </div>
                        <div className="flex flex-1 flex-col p-6">
                          ...
                          <span className="mt-5 inline-flex items-center gap-2 text-sm font-black transition group-hover:gap-3" style={{ color: palette.burgundy }}>
                            Read Article
                            <ArrowRight size={15} />
                          </span>
                        </div>
