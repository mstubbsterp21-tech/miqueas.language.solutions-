export function createMLSApi(session) {
  ...
  return {
    ...
    documentRequestCancel: (action = "cancel", method = "POST", body) => request("/api/document-request-cancel", action, method, body),
    opportunityEmail: (action = "send", method = "POST", body) => request("/api/opportunity-email", action, method, body),
