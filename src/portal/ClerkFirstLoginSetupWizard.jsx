...
export default function ClerkFirstLoginSetupWizard(props) {
  ...
  return <div className="relative">{source && <div className="fixed inset-x-3 top-[max(.75rem,env(safe-area-inset-top))] z-[80] flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-xl sm:left-auto sm:right-4 sm:max-w-sm"><FileCheck2 className="mt-0.5 shrink-0 text-emerald-700" size={18} /><div><p className="text-sm font-black text-emerald-950">Profile prefilled</p><p className="mt-1 text-xs leading-5 text-emerald-800">Matched your {source}. Review and edit every field before saving.</p></div></div>}{prefillError && <div className="fixed inset-x-3 top-[max(.75rem,env(safe-area-inset-top))] z-[80] rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold leading-5 text-amber-900 shadow-xl sm:left-auto sm:right-4 sm:max-w-md">{prefillError}</div>}<FirstLoginSetupWizard {...props} profile={resolvedProfile} /></div>;
}
