...
export function ClientAssignmentDetail({ assignment, actions }) {
  ...
  return <div className="space-y-6">
    ...
    <Card className="shadow-none">
      <SectionHeader title="Service at a glance" text="The details you need to prepare, arrive, and connect." />
    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <Card className="shadow-none"><SectionHeader title="Preparation checklist" text="Better preparation leads to smoother communication access." /><div className="mt-5 space-y-3">{checklist.map((item) => <div key={item.title} className="flex items-start gap-3 rounded-2xl bg-slate-50 p-4"><span className={cx("flex h-9 w-9 shrink-0 items-center justify-center rounded-xl", item.done ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700")}>{item.done ? <Check size={17} /> : <FileUp size={17} />}</span><div><p className="text-sm font-black text-slate-900">{item.title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{item.text}</p></div></div>)}</div><button type="button" onClick={() => document.querySelector("[data-assignment-document-center]")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 inline-flex items-center gap-2 text-sm font-black text-[#721100]"><FileUp size={16} />Open preparation documents</button></Card>
      <Card className="shadow-none"><SectionHeader title="Your service team" text="Confirmed names appear here when staffing is finalized." /><div className="mt-5 space-y-3">{team.map((link) => { const name = `${link.interpreters?.first_name || ""} ${link.interpreters?.last_name || ""}`.trim() || link.interpreters?.email || "MLS interpreter"; return <div key={link.id} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#721100]/10 font-black text-[#721100]">{name[0]}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-slate-900">{name}</p><p className="mt-1 text-xs text-slate-500">{pretty(link.role || "interpreter")}</p></div><Badge value={link.status || "assigned"} /></div>; })}{!team.length && <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center"><Users className="mx-auto text-[#721100]" size={24} /><p className="mt-3 font-black text-slate-900">Staffing in progress</p><p className="mt-1 text-xs leading-5 text-slate-500">MLS is matching credentials, setting experience, availability, and access needs.</p></div>}</div></Card>
    </div>

    <Card className="shadow-none"><SectionHeader title="Request details" text="The context MLS and the interpreting team use to prepare." /><div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-[.1em] text-slate-400">Service context</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{assignment.description || "No additional service context was provided."}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-[.1em] text-slate-400">Preparation notes</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{assignment.preparation_materials || "No preparation notes have been added yet."}</p></div></div></Card>

    <Card className="border-[#dd7d00]/20 bg-[#fffaf2] shadow-none"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#dd7d00]/15 text-[#9a5700]"><Headphones size={20} /></span><div><p className="font-black text-slate-950">Need help with this service?</p><p className="mt-1 text-sm leading-6 text-slate-600">Use Communications for changes, cancellation questions, access concerns, or day-of support so MLS has the request context.</p></div></div><button type="button" onClick={() => go("communications")} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-2xl bg-[#721100] px-4 py-3 text-sm font-black text-white"><MessageSquare size={16} />Message MLS</button></div></Card>
  </div>;
}

export function ClientRequestRoadmap({ actions }) {
  const steps = [
    [Sparkles, "Submit the full request", "Share timing, setting, communication preferences, participant details, and a day-of contact."],
    [CircleDollarSign, "Review scope and terms", "Approve the quote, sign the agreement, and complete any requested deposit."],
    [Users, "MLS staffs the service", "MLS matches modality, credentials, setting experience, availability, and location."],
    [FileUp, "Prepare the interpreting team", "Upload agendas, names, acronyms, slides, scripts, and links as early as possible."],
    [CheckCircle2, "Confirm and complete", "Use the portal for final logistics, service updates, billing, documents, and feedback."],
  ];
  return <Card><SectionHeader title="What happens after you request" text="A clear path from intake through closeout." action={<button type="button" onClick={actions.openRequest} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[#721100] px-4 py-2 text-xs font-black text-white"><ArrowRight size={14} />Start a request</button>} /><div className="mt-5 grid gap-3 md:grid-cols-5">{steps.map(([Icon, title, text], index) => <div key={title} className="relative rounded-2xl bg-slate-50 p-4"><div className="flex items-center justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#721100]/10 text-[#721100]"><Icon size={17} /></span><span className="text-2xl font-black text-slate-200">{index + 1}</span></div><p className="mt-4 text-sm font-black text-slate-900">{title}</p><p className="mt-2 text-xs leading-5 text-slate-500">{text}</p></div>)}</div></Card>;
}
