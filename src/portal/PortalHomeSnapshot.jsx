import { AlertCircle, Bell, CalendarDays, CircleDollarSign, ClipboardCheck, FileSignature, FileWarning, MessageSquare, ShieldCheck, Users } from "lucide-react";
...
function InterpreterHome({ workspace, operations, app, v2, actions }) {
  ...
  return <div className="space-y-6"><Hero title={`${profile.first_name || "Interpreter"}, here is what needs your attention.`} text="Your live snapshot of assignments, team updates, opportunities, documents, compliance, messages, and pay status." actions={<><Action tone="gold" onClick={() => actions.go("work")}>Open work center</Action><Action onClick={() => actions.go("communications")}>Open messages</Action></>} /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Metric icon={CalendarDays} name="Upcoming" value={upcoming.length} note="Assigned work" /><Metric icon={Bell} name="Unread" value={app?.unreadCount || 0} note="Portal notifications" color="#dd7d00" /><Metric icon={ClipboardCheck} name="Opportunities" value={opportunities.length} note="Open for bidding" color="#4338ca" /><Metric icon={FileWarning} name="Documents" value={docs.length} note="Requested files" color="#7c3aed" /><Metric icon={CircleDollarSign} name="Payments" value={payments.length} note="Ready or processing" color="#15803d" /></div><div className="grid gap-5 xl:grid-cols-2"><Card><SectionHeader title="Needs your attention" text="Preparation, compliance, files, and payment updates." /><div className="mt-5 space-y-3">{attention.map((item, index) => <AttentionItem key={`${item.title}-${index}`} {...item} onClick={item.action} />)}{!attention.length && <EmptyState icon={ShieldCheck} title="You are assignment-ready" text="No urgent work, document, or compliance items are waiting." />}</div></Card><Card><SectionHeader title="Next assignments" text="Open an assignment for team, preparation, location, and messages." /><div className="mt-5 space-y-3">{upcoming.slice(0, 6).map((item) => <AttentionItem key={item.id} title={item.service_type} text={`${formatDate(item.start_at)} · ${item.delivery_mode}`} badge={item.lifecycle_status || item.status} onClick={() => actions.openAssignment(item)} />)}{!upcoming.length && <EmptyState icon={CalendarDays} title="No upcoming assignments" text="Direct assignments and accepted bids will appear here." />}</div></Card></div></div>;
}
