...
const SPECIALTIES = ["General / Community", "Medical", "Legal", "Mental Health", "K-12 Education", "Post-Secondary Education", "Platform / Conference", "Performance / Arts", "Cruise"];
...
function AssignmentForm({ assignment, clients, actions, close }) {
  ...
  async function submit(event) {
    ...
    try {
      const payload = { ...draft, deaf_participants: draft.deaf_participants === "" ? null : Number(draft.deaf_participants), hearing_participants: draft.hearing_participants === "" ? null : Number(draft.hearing_participants) };
  ...
  return <form onSubmit={submit} className="space-y-5">{error && <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800">{error}</div>}
    ...
    <div className="sticky bottom-3 flex justify-end rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur"><button type="submit" disabled={saving || !clientId} className="inline-flex items-center gap-2 rounded-2xl bg-[#721100] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{saving ? <Loader2 className="animate-spin" size={16} /> : editing ? <Pencil size={16} /> : <CalendarPlus size={16} />}{editing ? "Save and sync assignment" : "Create and sync assignment"}</button></div>
  </form>;
}
