    ...
    <Card hover onClick={() => onOpen?.(assignment)} className={compact ? "p-4 md:p-4" : ""}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 truncate text-lg font-black text-slate-950">{assignment.service_type}</h3>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {fields.map(([label, value]) => (
        <div key={label} className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 whitespace-pre-wrap text-sm font-bold text-slate-700">{value || "Not provided"}</p>
        </div>
      ))}
    </div>
  );
...
export function ClientWorkspace({ section, workspace, operations, app, actions, busyDoc }) {
  ...
  if (section === "profile") return <Card><SectionHeader eyebrow="Account" title={profile.organization_name || "Client profile"} text="Contact, billing, service, and communication preferences." action={<Button icon={Settings2} onClick={actions.openProfile}>Edit profile</Button>} /><div className="mt-6"><ProfileSnapshot fields={[["Primary contact", profile.primary_contact_name], ["Email", profile.email], ["Phone", profile.phone], ["Preferred contact", profile.preferred_contact_method], ["Billing email", profile.billing_email], ["Location", [profile.city, profile.state].filter(Boolean).join(", ")], ["Default service", profile.default_service_type], ["Delivery", profile.default_delivery_mode], ["Communication preferences", profile.communication_preferences], ["Billing notes", profile.billing_notes]]} /></div></Card>;
