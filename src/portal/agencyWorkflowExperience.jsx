...
export function AgencyWorkflowCard({ currentStage = -1, compact = false }) {
  if (currentStage < 0) return null;
  return <Card className={compact ? "p-4 md:p-5" : ""}><SectionHeader title="Service progress" /><div className="mt-4 grid gap-2 sm:grid-cols-3 xl:grid-cols-6">{WORKFLOW_STEPS.map((step, index) => { const Icon = step.icon; const active = index === currentStage; const complete = index < currentStage; return <div key={step.key} className={cx("rounded-2xl border p-3", active ? "border-[#dd7d00] bg-[#fff7e8] shadow-sm" : complete ? "border-emerald-200 bg-emerald-50/70" : "border-slate-200 bg-slate-50")}><span className={cx("flex h-9 w-9 items-center justify-center rounded-xl", active ? "bg-[#721100] text-white" : complete ? "bg-emerald-600 text-white" : "bg-white text-slate-500")}><Icon size={16} /></span><p className="mt-3 text-xs font-black text-slate-900">{step.label}</p></div>; })}</div></Card>;
}
...
export function InterpreterAssignmentDetail({ assignment, v2, viewerInterpreterId, actions }) {
  ...
  return <div className="space-y-6"><div className="relative overflow-hidden rounded-[1.6rem] bg-gradient-to-br from-[#721100] via-[#5b180b] to-[#24130e] p-5 text-white sm:p-7"><span className="absolute -right-12 -top-16 h-48 w-48 rounded-full border-[28px] border-white/5" /><div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-black sm:text-3xl">{assignment.service_type || "Interpreter service"}</h2><p className="mt-3 text-sm text-white/70">{formatDate(assignment.start_at)} · {assignment.delivery_mode || "Delivery pending"}</p></div><div className="flex flex-wrap gap-2"><Badge value={assignment.lifecycle_status || assignment.status} className="border-white/20 bg-white/10 text-white" />{ownLink?.status && <Badge value={ownLink.status} className="border-white/20 bg-white/10 text-white" />}</div></div></div><AgencyWorkflowCard currentStage={meta.stage} compact /><Card><div className={cx("rounded-2xl border p-4", TONE[meta.tone])}><p className="text-[10px] font-black uppercase tracking-[.12em] opacity-70">Next action</p><p className="mt-2 text-lg font-black">{meta.title}</p><p className="mt-1 text-sm leading-6 opacity-80">{meta.next}</p></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{assignment.delivery_mode === "VRI" && assignment.meeting_link && <a href={assignment.meeting_link} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-[#721100] px-4 text-sm font-black text-white">Join meeting <ExternalLink size={14} /></a>}{directions && <a href={directions} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-[#721100] px-4 text-sm font-black text-white">Directions <MapPin size={14} /></a>}<button type="button" onClick={() => downloadCalendar(assignment)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-black text-[#721100]"><CalendarDays size={14} />Add to calendar</button><button type="button" onClick={() => navigate("communications")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-black text-[#721100]"><MessageSquare size={14} />Message MLS</button>{meta.stage >= 4 && <button type="button" onClick={() => navigate("payments")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 text-sm font-black text-[#721100]"><UploadCloud size={14} />Time & Pay</button>}</div></Card><Card><SectionHeader title="Service details" /><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Location or access", location], ["Participants", `${assignment.deaf_participants || 0} Deaf · ${assignment.hearing_participants || 0} hearing`], ["Setting", assignment.specialty || assignment.setting || "Not specified"], ["Language and access", assignment.language_preferences || assignment.communication_preferences || "Not specified"], ["Role", pretty(ownLink?.role || "interpreter")], ["Confirmation", pretty(ownLink?.status || assignment.status)], ["Dress or arrival", assignment.dress_code || assignment.arrival_instructions || "Not specified"], ["Day-of contact", assignment.onsite_contact_name || assignment.contact_name || "Not listed"]].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-[.1em] text-slate-400">{label}</p><p className="mt-2 break-words text-sm font-bold leading-6 text-slate-700">{value}</p></div>)}</div></Card><Card><SectionHeader title="Preparation" /><div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-[.1em] text-[#721100]">Context</p><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-700">{assignment.description || "No context added."}</p></div><div className="rounded-2xl bg-[#fff7e8] p-4"><p className="text-xs font-black uppercase tracking-[.1em] text-[#721100]">Materials</p><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-700">{assignment.preparation_materials || "No materials added."}</p></div></div></Card><Card><SectionHeader title="Team" /><div className="mt-5 grid gap-3 sm:grid-cols-2">{team.map((link) => <div key={link.id} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#721100]/10 font-black text-[#721100]">{(link.interpreters?.first_name || "I")[0]}</span><div className="min-w-0 flex-1"><p className="font-black text-slate-900">{`${link.interpreters?.first_name || ""} ${link.interpreters?.last_name || ""}`.trim() || (link.interpreter_id === viewerInterpreterId ? "You" : "Assigned interpreter")}</p><p className="mt-1 text-xs text-slate-500">{pretty(link.role || "interpreter")}</p></div><Badge value={link.status || "assigned"} /></div>)}{!team.length && <p className="text-sm text-slate-500">Team pending.</p>}</div></Card><div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-900"><strong>Private:</strong> Client billing, agency margin, administrative notes, other interpreters’ rates, and compliance records are restricted.</div></div>;
}
...
export function AdminLifecyclePanel({ assignment }) {
  ...
  return <AgencyWorkflowCard currentStage={stage} compact />;
}
