...
export function Badge({ value, className = "" }) {
  return (
    <span className={cx(
      "inline-flex shrink-0 items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[.08em]",
      badgeStyles[value] || "border-slate-200 bg-slate-50 text-slate-600",
      className,
    )}>
      {pretty(value)}
    </span>
...
export function Card({ children, className = "", hover = false, onClick }) {
  ...
  return (
    <Element
      ...
      onClick={onClick}
      className={cx(
        "rounded-[1.75rem] border border-black/5 bg-white p-5 text-left shadow-[0_18px_55px_rgba(40,25,18,.07)] md:p-6",
...
export function Metric({ icon: Icon, name, value, note, color = BRAND.burgundy, onClick }) {
  return (
    <motion.button
      ...
      whileHover={{ y: -3 }}
      className={cx(
        "rounded-[1.55rem] border border-black/5 bg-white p-5 text-left shadow-[0_16px_50px_rgba(40,25,18,.07)]",
        onClick ? "cursor-pointer" : "cursor-default",
      )}
    >
      <div className="flex justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">{name}</p>
...
export function SectionHeader({ eyebrow, title, text, action }) {
  return (
    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <p className="text-xs font-black uppercase tracking-[.16em] text-[#dd7d00]">{eyebrow}</p>
        <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950 md:text-3xl">{title}</h2>
        {text && <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">{text}</p>}
      </div>
      {action}
    </div>
  );
...
export function Hero({ eyebrow, title, text, actions, children }) {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.99 }}
      animate={{ opacity: 1, scale: 1 }}
      className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#721100] via-[#5b180b] to-[#24130e] p-6 text-white shadow-2xl md:p-8"
    >
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[.18em] text-[#f6b34c]">{eyebrow}</p>
          <h1 className="mt-3 max-w-4xl text-3xl font-black tracking-tight md:text-5xl">{title}</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-white/72">{text}</p>
          {children}
        </div>
...
export function Field({ name, required, children, className = "", help }) {
  return (
    <label className={className}>
      <span className="mb-2 block text-xs font-black uppercase tracking-[.1em] text-slate-500">
        {name}{required && <b className="text-[#721100]"> *</b>}
      </span>
      {children}
...
export function EmptyState({ icon: Icon = FileText, title, text, action }) {
  return (
    <div className="rounded-[1.55rem] border border-dashed border-slate-300 bg-white/60 px-6 py-10 text-center">
      <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-[#721100]/10 text-[#721100]">
        <Icon size={22} />
      </span>
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[120] flex items-center justify-center bg-[#1c100c]/70 p-3 backdrop-blur-sm md:p-5"
        ...
        >
          <motion.div
            ...
            className={cx(
              "max-h-[94vh] w-full overflow-y-auto rounded-[2rem] bg-[#f8f5f1] shadow-2xl",
              wide ? "max-w-6xl" : "max-w-3xl",
            )}
          >
            <div className="sticky top-0 z-20 flex items-start justify-between border-b border-black/5 bg-[#f8f5f1]/95 px-5 py-5 backdrop-blur md:px-7">
              <div>
                <h2 className="text-2xl font-black text-slate-950">{title}</h2>
                {subtitle && <p className="mt-1 text-sm text-slate-500">{subtitle}</p>}
              </div>
              <button type="button" onClick={close} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-slate-500 shadow">
                <X size={18} />
              </button>
            </div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">{request?.instructions || "Upload the latest requested version."}</p>
          </div>
        </div>
...
export function Toast({ message, type = "success", dismiss }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -8 }}
      className={cx(
        "mb-5 flex items-start gap-3 rounded-2xl border p-4 text-sm shadow-sm",
        success ? "border-emerald-200 bg-emerald-50 font-bold text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800",
      )}
    >
      {success ? <CheckCircle2 size={18} /> : <Bell size={18} />}
      <span className="flex-1">{message}</span>
      {dismiss && <button type="button" onClick={dismiss}><X size={16} /></button>}
    </motion.div>
  );
}
...
export function InstallAppButton({ compact = false }) {
  ...
  return (
    <button
      ...
      className={cx(
        "inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 font-black text-white/80 transition hover:bg-white/10 hover:text-white",
        compact ? "h-10 w-10" : "w-full px-4 py-3 text-sm",
      )}
      title="Install MLS app"
    >
      <Smartphone size={16} />
      {!compact && "Install app"}
    </button>
...
export function FloatingAction({ onClick, label, icon: Icon = Plus }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className="fixed bottom-24 right-5 z-40 inline-flex items-center gap-2 rounded-full bg-[#721100] px-5 py-3.5 text-sm font-black text-white shadow-2xl md:bottom-8 md:right-8"
    >
      <Icon size={18} /> {label}
    </motion.button>
  );
