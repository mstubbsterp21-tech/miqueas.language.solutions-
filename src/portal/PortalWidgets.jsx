...
import {
  ...
  Clock3, Droplets, ExternalLink, Loader2, LocateFixed, MapPinned, Moon,
  Newspaper, RefreshCcw, Sun, Wind,
} from "lucide-react";
...
function ClockWidget({ now, timeZone }) {
  ...
  return <Card className="overflow-hidden"><SectionHeader title="Date & time" text={timeZoneLabel(timeZone)} /><div className="relative mt-5 overflow-hidden rounded-[1.6rem] bg-gradient-to-br from-[#721100] via-[#98200d] to-[#dd7d00] p-5 text-white"><span className="absolute -right-8 -top-10 h-32 w-32 rounded-full bg-white/10" /><span className="absolute -bottom-12 left-16 h-28 w-28 rounded-full bg-[#f7c876]/20" /><div className="relative flex items-center gap-4"><div className="w-20 shrink-0 overflow-hidden rounded-2xl bg-white text-center shadow-xl"><p className="bg-[#24130e] py-1.5 text-[10px] font-black uppercase tracking-[.18em] text-[#f6b34c]">{month}</p><p className="py-2 text-3xl font-black text-[#721100]">{day}</p></div><div className="min-w-0"><div className="flex items-center gap-2 text-white/65"><Clock3 size={15} /><span className="text-[10px] font-black uppercase tracking-[.16em]">Local time</span></div><p className="mt-1 break-words text-2xl font-black sm:text-3xl">{time}</p><p className="mt-1 text-sm font-bold text-white/75">{weekday}</p></div></div></div><p className="mt-3 flex items-center gap-2 text-xs font-bold text-slate-500"><CalendarDays size={14} className="text-[#dd7d00]" />{date}</p></Card>;
}
