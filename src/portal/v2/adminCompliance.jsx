...
export default function AdminComplianceV2({ workspace, operations, v2, actions, saving }) {
  ...
  return (
    <div className="space-y-6">
      <Hero title="Compliance" actions={<ActionButton tone="gold" onClick={actions.openDocumentRequest}>Request document</ActionButton>} />
      <Card>
        <SectionHeader title={`Document requests (${documentRequests.length})`} action={<ActionButton onClick={actions.openDocumentRequest}>New request</ActionButton>} />
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card>
          <SectionHeader title="Credential record" />
        ...
        <Card>
          <SectionHeader title="Onboarding record" />
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card><SectionHeader title={`Expiring within 90 days (${expiring.length})`} /><div className="mt-5 space-y-3">{expiring.map((item) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{item.credential_name}</p><p className="mt-1 text-xs text-slate-500">{item.interpreters?.first_name} {item.interpreters?.last_name} · {item.expires_on || "No expiration"}</p></div><Badge value={item.verification_status} /></div></div>)}{!expiring.length && <EmptyState icon={ShieldCheck} title="No upcoming expirations" />}</div></Card>
        <Card><SectionHeader title={`Interpreter onboarding (${pipelines.length})`} /><div className="mt-5 space-y-3">{pipelines.map((item) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{item.interpreters?.first_name} {item.interpreters?.last_name}</p><p className="mt-1 text-xs text-slate-500">{item.stage.replaceAll("_", " ")} · Due {item.due_date || "not set"}</p></div><Badge value={item.status} /></div></div>)}{!pipelines.length && <EmptyState icon={UserCheck} title="No onboarding records" />}</div></Card>
      </div>
      <Card><SectionHeader title={`Training library (${courses.length})`} action={<ActionButton onClick={actions.openCourse}>Add course</ActionButton>} /><div className="mt-5 grid gap-3 md:grid-cols-2">{courses.map((course) => <div key={course.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{course.title}</p><p className="mt-1 text-xs text-slate-500">{course.category} · {course.duration_minutes ? `${course.duration_minutes} min` : "Self-paced"}</p></div><Badge value={course.is_published ? "published" : "draft"} /></div></div>)}{!courses.length && <EmptyState icon={GraduationCap} title="No courses" />}</div></Card>
    </div>
  );
}
