...
export function ActionButton({ children, onClick, type = "button", tone = "primary", disabled = false, icon: Icon }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={cx("inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50", styles[tone])}>
...
export function AssignmentRow({ assignment, onOpen, detail }) {
  return (
    <button type="button" onClick={() => onOpen?.(assignment)} className="flex w-full flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-[#dd7d00]/40 hover:bg-white hover:shadow-lg md:flex-row md:items-center">
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate font-black text-slate-950">{assignment.service_type || "MLS assignment"}</span>
      ...
      <span className="flex flex-wrap items-center gap-2"><Badge value={assignment.lifecycle_status || assignment.status} /><ArrowRight size={16} className="text-slate-400" /></span>
    </button>
  );
}
