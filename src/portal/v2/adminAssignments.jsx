...
import { AgencyWorkflowCard, AdminOperationsCard, adminAssignmentMeta, recordsForAssignment } from "../agencyWorkflowExperience";
import { Card, EmptyState, Hero, INPUT, Metric, SectionHeader, cx } from "../ui";
...
export default function AdminAssignmentsV2({ workspace, app, v2, actions }) {
  ...
  return <div className="space-y-6">
    <Hero eyebrow="Agency operations" title="Service Operations" text="Move every request from client intake through authorization, staffing, service delivery, closeout, invoicing, contractor pay, and archive—without losing the handoff." actions={<div className="flex flex-wrap gap-3"><NewAssignmentControl clients={clients} actions={actions} /><ActionButton tone="soft" onClick={actions.openOpportunity}>Publish opportunity</ActionButton></div>} />
    ...
    <AgencyWorkflowCard role="admin" />
    <Card><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><SectionHeader title="Operational queue" text="Each card explains the next agency action and keeps quote, agreement, staffing, service, and invoice status together." /><div className="flex min-w-0 flex-col gap-2 lg:flex-row"><label className="flex min-h-11 min-w-0 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 lg:w-72"><Search size={16} className="shrink-0 text-[#dd7d00]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search client or service" className="min-w-0 flex-1 bg-transparent text-sm font-bold outline-none" /></label><div className="flex max-w-full items-center gap-1 overflow-x-auto rounded-2xl border border-slate-200 bg-slate-50 p-1.5"><Filter size={15} className="ml-2 shrink-0 text-slate-400" />{FILTERS.map(([value, label]) => <button key={value} type="button" onClick={() => setView(value)} className={cx("min-h-9 whitespace-nowrap rounded-xl px-3 text-xs font-black", view === value ? "bg-[#721100] text-white" : "text-slate-500")}>{label}</button>)}</div></div></div><div className="mt-6 space-y-4">{visible.map(({ assignment, records }) => <AdminOperationsCard key={assignment.id} assignment={assignment} records={records} onOpen={actions.openAssignment} />)}{!visible.length && <EmptyState icon={Search} title="No services match this view" text="Change the filter or create a new assignment from a client request." />}</div></Card>
  </div>;
}
