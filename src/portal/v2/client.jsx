      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card>
          ...
          <div className="mt-5 space-y-4">
            {quotes.map((quote) => <div key={quote.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="flex justify-between gap-3"><div><p className="font-black text-slate-950">{quote.quote_number}</p><p className="mt-1 text-xs text-slate-500">{quote.assignments?.service_type} · {formatDate(quote.assignments?.start_at)}</p></div><Badge value={quote.status} /></div><div className="mt-4 space-y-2">{(quote.quote_items || []).map((item) => <div key={item.id} className="flex justify-between gap-4 text-sm"><span>{item.description} · {item.quantity} × {formatMoney(item.unit_rate)}</span><b>{formatMoney(item.amount)}</b></div>)}</div><div className="mt-4 flex items-end justify-between border-t border-slate-200 pt-4"><div><p className="text-xs text-slate-400">Total</p><p className="text-2xl font-black">{formatMoney(quote.total_amount)}</p>{Number(quote.deposit_amount || 0) > 0 && <p className="mt-1 text-xs text-slate-500">Deposit: {formatMoney(quote.deposit_amount)}</p>}</div>{quote.status === "sent" && <div className="flex flex-wrap justify-end gap-2"><ActionButton disabled={saving} onClick={() => actions.respondQuote(quote.id, "approved")}>Approve</ActionButton><ActionButton disabled={saving} tone="soft" onClick={() => actions.respondQuote(quote.id, "changes_requested", window.prompt("What should MLS change?") || "Changes requested")}>Request changes</ActionButton><ActionButton disabled={saving} tone="danger" onClick={() => actions.respondQuote(quote.id, "rejected")}>Decline</ActionButton></div>}</div></div>)}
