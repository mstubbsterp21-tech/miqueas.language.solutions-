...
function ClientHome({ workspace, app, v2, actions }) {
  ...
  return <div className="space-y-6">
    <Hero title={profile.organization_name ? `Welcome, ${profile.organization_name}` : "Your interpreting service hub"} text="Request services, make approvals, prepare the interpreting team, and follow every service through billing." actions={<><ActionButton tone="gold" icon={Plus} onClick={actions.openRequest}>Request interpreter</ActionButton><ActionButton tone="soft" icon={MessageSquare} onClick={() => messageMLS(actions)}>Message MLS</ActionButton></>} />
    ...
    {upcoming[0] && <section><SectionHeader title="Your next service" text="Open it for confirmation status, preparation, directions, calendar access, documents, and support." /><div className="mt-4"><ClientAssignmentCard assignment={upcoming[0]} onOpen={actions.openAssignment} onRepeat={actions.openRepeatRequest} onMessage={() => messageMLS(actions)} /></div></section>}
    <ClientRequestRoadmap actions={actions} />
  </div>;
}
...
function ClientRequests({ workspace, app, v2, actions, saving }) {
  ...
  return <div className="space-y-6">
    <Hero title="Requests and approvals" text="See what needs a decision, where every request stands, and what happens next." actions={<><ActionButton tone="gold" icon={Plus} onClick={actions.openRequest}>New request</ActionButton><ActionButton tone="soft" icon={MessageSquare} onClick={() => messageMLS(actions)}>Ask MLS a question</ActionButton></>} />
    ...
    {(waitingQuotes.length > 0 || waitingAgreements.length > 0) && <Card><SectionHeader title="Decisions waiting for you" text="Complete these items so MLS can keep the service moving." /><div className="mt-5 grid gap-5 xl:grid-cols-2"><div className="space-y-4">{waitingQuotes.map((item) => <QuoteCard key={item.id} quote={item} actions={actions} saving={saving} />)}</div><div className="space-y-4">{waitingAgreements.map((item) => <AgreementCard key={item.id} agreement={item} actions={actions} />)}</div></div></Card>}
    <Card>
      <SectionHeader title="Request tracker" text="Search every service and open one for status, preparation, logistics, documents, and support." />
      ...
      <div className="mt-5 space-y-4">{filtered.map((item) => <ClientAssignmentCard key={item.id} assignment={item} onOpen={actions.openAssignment} onRepeat={actions.openRepeatRequest} onMessage={() => messageMLS(actions)} />)}{!filtered.length && <EmptyState icon={Search} title="No requests match" text="Try another filter or submit a new interpreting request." action={<ActionButton icon={Plus} onClick={actions.openRequest}>New request</ActionButton>} />}</div>
    </Card>
    {(quotes.length > waitingQuotes.length || agreements.length > waitingAgreements.length) && <Card><SectionHeader title="Approval history" text="Previously approved, signed, declined, or expired records remain available." /><div className="mt-5 grid gap-5 xl:grid-cols-2"><div className="space-y-4">{quotes.filter((item) => item.status !== "sent").map((item) => <QuoteCard key={item.id} quote={item} actions={actions} saving={saving} />)}{!quotes.filter((item) => item.status !== "sent").length && <EmptyState icon={ReceiptText} title="No quote history" text="Completed quote decisions will remain here." />}</div><div className="space-y-4">{agreements.filter((item) => !["sent", "viewed"].includes(item.status)).map((item) => <AgreementCard key={item.id} agreement={item} actions={actions} />)}{!agreements.filter((item) => !["sent", "viewed"].includes(item.status)).length && <EmptyState icon={FileSignature} title="No agreement history" text="Completed agreements will remain here." />}</div></div></Card>}
    <ClientRequestRoadmap actions={actions} />
  </div>;
}
...
function ClientAssignments({ workspace, app, actions }) {
  ...
  return <div className="space-y-6"><Hero title="Services" text="Your live schedule and complete interpreting service history." actions={<><ActionButton tone="gold" icon={Plus} onClick={actions.openRequest}>New request</ActionButton><ActionButton tone="soft" icon={MessageSquare} onClick={() => messageMLS(actions)}>Message MLS</ActionButton></>} /><Card><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><SectionHeader title="Service schedule" text="Open any service for its progress, access details, team, preparation, documents, and calendar link." /><div className="flex flex-col gap-2 sm:flex-row"><input className={cx(INPUT, "sm:w-72")} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search services" /><div className="flex gap-1 rounded-2xl border border-slate-200 bg-slate-50 p-1.5">{[["upcoming", "Upcoming"], ["active", "Active"], ["history", "History"], ["all", "All"]].map(([value, label]) => <button key={value} type="button" onClick={() => setView(value)} className={cx("min-h-9 rounded-xl px-3 text-xs font-black", view === value ? "bg-[#721100] text-white" : "text-slate-500")}>{label}</button>)}</div></div></div><div className="mt-6 space-y-4">{visible.map((item) => <ClientAssignmentCard key={item.id} assignment={item} onOpen={actions.openAssignment} onRepeat={actions.openRepeatRequest} onMessage={() => messageMLS(actions)} />)}{!visible.length && <EmptyState icon={CalendarDays} title="No services in this view" text="Change the filter or submit a new interpreting request." action={<ActionButton icon={Plus} onClick={actions.openRequest}>Request interpreter</ActionButton>} />}</div></Card></div>;
}
...
function ClientBilling({ v2, actions }) {
  ...
  return <div className="space-y-6"><Hero title="Billing and payments" text="See every invoice, due date, payment status, and service reference in one place." actions={<ActionButton tone="soft" icon={MessageSquare} onClick={() => messageMLS(actions)}>Billing question</ActionButton>} /><div className="grid gap-4 sm:grid-cols-3"><Metric icon={WalletCards} name="Balance due" value={formatMoney(due)} note={`${open.length} open invoice${open.length === 1 ? "" : "s"}`} color="#c2410c" /><Metric icon={Clock3} name="Overdue" value={overdue.length} note={overdue.length ? `${formatMoney(overdue.reduce((sum, item) => sum + Number(item.balance_due || 0), 0))} past due` : "Nothing past due"} color="#be123c" /><Metric icon={CheckCircle2} name="Payments recorded" value={formatMoney(paid)} note="Across portal invoices" color="#15803d" /></div><Card><SectionHeader title="Invoices" text={`${invoices.length} invoice record${invoices.length === 1 ? "" : "s"}. Found remains the payment and accounting record when a link is available.`} /><div className="mt-5 space-y-4">{invoices.map((item) => <div key={item.id} className={cx("rounded-2xl border p-4 sm:p-5", Number(item.balance_due || 0) > 0 ? "border-orange-200 bg-orange-50/50" : "border-slate-200 bg-slate-50")}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 gap-3"><span className={cx("flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl", Number(item.balance_due || 0) > 0 ? "bg-orange-100 text-orange-700" : "bg-emerald-100 text-emerald-700")}><ReceiptText size={19} /></span><div><p className="font-black text-slate-950">Invoice {item.invoice_number}</p><p className="mt-1 text-xs leading-5 text-slate-500">{item.assignments?.service_type || "Interpreter service"} · {formatDate(item.assignments?.start_at)}</p><p className={cx("mt-1 text-xs font-bold", invoiceTiming(item).includes("overdue") ? "text-rose-700" : "text-slate-500")}>{invoiceTiming(item)} · Due {item.due_date || "—"}</p></div></div><Badge value={item.status} /></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><MoneySummary label="Invoice total" value={item.total_amount} /><MoneySummary label="Paid" value={item.amount_paid} /><MoneySummary label="Balance" value={item.balance_due} /></div>{item.memo && <p className="mt-4 rounded-xl bg-white px-3 py-2 text-xs leading-5 text-slate-600"><b>Memo:</b> {item.memo}</p>}<div className="mt-4 flex flex-wrap items-center gap-4"><ExternalRecordLink href={item.payment_link || item.found_invoice_url}>{item.payment_link ? "Open payment link" : "Open in Found"}</ExternalRecordLink>{item.assignments?.lifecycle_status && <Badge value={item.assignments.lifecycle_status} />}</div></div>)}{!invoices.length && <EmptyState icon={ShieldCheck} title="No invoices" text="Invoices and payment history will appear here when available." />}</div></Card><Card className="border-blue-100 bg-blue-50/50"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-100 text-blue-700"><Sparkles size={19} /></span><div><p className="font-black text-slate-950">Keep billing smooth</p><p className="mt-1 text-sm leading-6 text-slate-600">Add a PO number or internal reference to your request, keep the billing email current in Profile, and message MLS before service if your organization needs special invoice documentation.</p></div></div></Card></div>;
}
