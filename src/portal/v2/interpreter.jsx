...
import { AgencyWorkflowCard, InterpreterWorkCard, recordsForAssignment } from "../agencyWorkflowExperience";
...
function Home({ workspace, operations, app, v2, actions }) {
  ...
  return (
    <div className="space-y-6">
      <Hero eyebrow="Interpreter workspace" title={`${profile.first_name || "Interpreter"}, keep your MLS work assignment-ready.`} text="Matched opportunities, confirmed services, preparation, availability, closeout submissions, and your payment records follow one clear work cycle." actions={<ActionButton tone="gold" onClick={() => actions.go("work")}>Open work center</ActionButton>} />
      ...
      <AgencyWorkflowCard role="interpreter" />
      <Card><SectionHeader eyebrow="Next up" title="Upcoming assignments" text="Open an assignment for role-safe preparation, location, messages, calendar tools, time, and payment status." /><div className="mt-5 grid gap-4 xl:grid-cols-2">{upcoming.slice(0, 6).map((item) => <InterpreterWorkCard key={item.id} assignment={item} records={recordsForAssignment(v2, item.id)} onOpen={actions.openAssignment} onTimePay={() => actions.go("payments")} />)}{!upcoming.length && <EmptyState icon={CalendarDays} title="No upcoming assignments" text="Accepted bids and direct assignments will appear here." />}</div></Card>
    </div>
  );
}
...
function Work({ operations, app, v2, actions }) {
  ...
  return (
    <div className="space-y-6">
      <Hero eyebrow="Interpreter work center" title="Opportunities and Assigned Services" text="Opportunities contain only the information needed to decide whether you are qualified and available. Assigned services unlock the preparation, logistics, team, closeout, and personal payment details connected to your work." actions={<ActionButton tone="soft" onClick={() => actions.go("schedule")}>Update availability</ActionButton>} />
      <AgencyWorkflowCard role="interpreter" />
      <Card><SectionHeader title="Matched opportunities" text="Review schedule, modality, general location, setting, participants, language, and team requirements before expressing interest. Client billing and internal agency details stay private." action={viewToggle(opportunityView, setOpportunityView)} /><div className="mt-5">{opportunityView === "list" ? <div className="grid gap-4 lg:grid-cols-2">{opportunities.map((opportunity) => { const item = opportunity.assignments || {}; return <button type="button" key={opportunity.id} onClick={() => actions.submitBid(opportunity)} className="group min-w-0 overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#dd7d00]/50 hover:shadow-lg"><div className="bg-gradient-to-br from-[#721100] to-[#24130e] p-4 text-white"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.12em] text-[#f6b34c]">Matched opportunity</p><p className="mt-1 break-words text-lg font-black">{item.service_type || "Interpreter opportunity"}</p><p className="mt-2 break-words text-xs leading-5 text-white/70">{formatDate(item.start_at)} · {item.delivery_mode || "Delivery pending"}</p></div><Badge value={opportunity.status} className="border-white/20 bg-white/10 text-white" /></div></div><div className="grid gap-2 p-4 text-xs text-slate-600 sm:grid-cols-2"><p><b>Area:</b> {item.delivery_mode === "VRI" ? "Remote" : [item.city, item.state].filter(Boolean).join(", ") || "Shared after assignment"}</p><p><b>Setting:</b> {item.specialty || item.setting || "General"}</p><p><b>Participants:</b> {item.deaf_participants || 0} Deaf · {item.hearing_participants || 0} hearing</p><p><b>Team:</b> {item.team_requested ? "Team requested" : "Solo assignment"}{item.cdi_requested ? " · CDI" : ""}</p><p className="sm:col-span-2"><b>Language/access:</b> {item.language_preferences || item.communication_preferences || "Review details"}</p><p className="inline-flex items-center gap-1 font-black text-[#721100] sm:col-span-2">Review and express interest <ClipboardCheck size={13} /></p></div></button>; })}{!opportunities.length && <EmptyState icon={ClipboardCheck} title="No recommended opportunities" text="New work will appear when it matches your profile, availability, and required documents." />}</div> : <AssignmentCalendar assignments={opportunityAssignments} onOpen={(item) => actions.submitBid(item.opportunity)} />}</div></Card>
      <Card><SectionHeader title="Assigned services" text="Confirmed work with the preparation, access, team, day-of, time, and payment information you need." action={viewToggle(view, setView)} /><div className="mt-5">{view === "list" ? <div className="grid gap-4 xl:grid-cols-2">{assignments.map((item) => <InterpreterWorkCard key={item.id} assignment={item} records={recordsForAssignment(v2, item.id)} onOpen={actions.openAssignment} onTimePay={() => actions.go("payments")} />)}{!assignments.length && <EmptyState icon={CalendarDays} title="No assigned work" text="Accepted opportunities and direct assignments will appear here." />}</div> : <AssignmentCalendar assignments={assignments} onOpen={actions.openAssignment} />}</div></Card>
    </div>
  );
}
...
function Payments({ app, v2, actions, saving }) {
  ...
  return (
    <div className="space-y-6">
      <Hero eyebrow="Service closeout" title="Time, Expenses & Pay" text="Complete the records MLS needs to close the client service, prepare contractor payment, and preserve your own payment history. You see only your submissions and payments." />
      ...
      <AgencyWorkflowCard role="interpreter" currentStage={completedNeedingTime.length ? 4 : scheduledPayments.length ? 5 : -1} />
      {completedNeedingTime.length > 0 && <Card><SectionHeader title="Closeout needed" text="Submit actual time promptly so MLS can verify the client record, invoice accurately, and prepare contractor payment." /><div className="mt-5 grid gap-3 md:grid-cols-2">{completedNeedingTime.map((assignment) => <button key={assignment.id} type="button" onClick={() => startTimeEntry(assignment)} className="flex items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-left"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-100 text-rose-700"><Clock3 size={17} /></span><span className="min-w-0 flex-1"><span className="block font-black text-slate-900">{assignment.service_type}</span><span className="mt-1 block text-xs text-slate-500">{formatDate(assignment.start_at)} · Add actual time</span></span></button>)}</div></Card>}
      <div className="grid gap-5 xl:grid-cols-2">
        <div id="interpreter-time-entry"><Card><SectionHeader title="Submit actual time" text={`Enter both times in ${timeZoneLabel(timeZone)}. Use notes for approved extensions, minimums, or service issues.`} /><form onSubmit={submitTime} className="mt-6 grid gap-4"><Field name="Assignment" required><SelectField value={time.assignmentId} onChange={(event) => setTime({ ...time, assignmentId: event.target.value })} options={assignmentOptions} /></Field><div className="grid gap-3 sm:grid-cols-2"><Field name={`Actual start · ${zone}`} required><input className={INPUT} type="datetime-local" value={time.actualStartAt} onChange={(event) => setTime({ ...time, actualStartAt: event.target.value })} /></Field><Field name={`Actual end · ${zone}`} required><input className={INPUT} type="datetime-local" value={time.actualEndAt} onChange={(event) => setTime({ ...time, actualEndAt: event.target.value })} /></Field></div><Field name="Break minutes"><input className={INPUT} type="number" min="0" value={time.breakMinutes} onChange={(event) => setTime({ ...time, breakMinutes: event.target.value })} /></Field><Field name="Notes"><textarea className={INPUT} rows={3} value={time.notes} onChange={(event) => setTime({ ...time, notes: event.target.value })} /></Field><ActionButton type="submit" disabled={saving || !time.assignmentId || !time.actualStartAt || !time.actualEndAt}>Submit time for review</ActionButton></form></Card></div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card><SectionHeader eyebrow="Your submissions" title="Time entries" text="Follow MLS review and correction status. Client billing values are not shown here." /><div className="mt-5 space-y-3">{timeEntries.map((item) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{item.assignments?.service_type}</p><p className="mt-1 text-xs text-slate-500">{item.billable_hours} hours · {formatDate(item.actual_start_at)}</p>{item.review_notes && <p className="mt-2 text-xs leading-5 text-slate-600"><b>MLS note:</b> {item.review_notes}</p>}</div><Badge value={item.status} /></div></div>)}{!timeEntries.length && <EmptyState icon={Clock3} title="No time submitted" text="Completed assignment time will appear here." />}</div></Card>
        <Card><SectionHeader eyebrow="Your contractor pay" title="Payment status" text="Only your contractor payment amounts and Found references are visible. Client invoices, agency margin, and other interpreters’ payments stay private." /><div className="mt-5 space-y-3">{payments.map((item) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{item.assignments?.service_type}</p><p className="mt-1 text-xs text-slate-500">{formatMoney(item.contractor_payment_amount)} · Due {item.contractor_payment_due_date || "not set"}</p></div><Badge value={item.contractor_payment_status} /></div>{item.found_payment_url && <a href={item.found_payment_url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-xs font-black text-[#721100] hover:underline">Open your payment record in Found</a>}</div>)}{!payments.length && <EmptyState icon={CircleDollarSign} title="No payment records" text="Your Found payment reference appears after MLS approves closeout and prepares payment." />}</div></Card>
      </div>
