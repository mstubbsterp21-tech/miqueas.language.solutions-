...
function Work({ operations, app, actions }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><SectionHeader title="Recommended Opportunities" text="Review the service, schedule, general location, participants, language, setting, and teaming needs before expressing interest." action={viewToggle(opportunityView, setOpportunityView)} /><div className="mt-5">{opportunityView === "list" ? <div className="grid gap-3 lg:grid-cols-2">{opportunities.map((opportunity) => { const item = opportunity.assignments || {}; return <button type="button" key={opportunity.id} onClick={() => actions.submitBid(opportunity)} className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left transition hover:border-[#dd7d00]/50 hover:bg-white hover:shadow-lg"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-black text-slate-950">{item.service_type || "Interpreter opportunity"}</p><p className="mt-1 break-words text-xs leading-5 text-slate-500">{formatDate(item.start_at)} · {item.delivery_mode || "Delivery pending"}{item.city || item.state ? ` · ${[item.city, item.state].filter(Boolean).join(", ")}` : ""}</p></div><Badge value={opportunity.status} /></div><p className="mt-4 text-xs font-black text-[#721100]">View Details →</p></button>; })}{!opportunities.length && <EmptyState icon={ClipboardCheck} title="No recommended opportunities" text="New work will appear when it matches your profile, availability, and required documents." />}</div> : <AssignmentCalendar assignments={opportunityAssignments} onOpen={(item) => actions.submitBid(item.opportunity)} />}</div></Card>
      <Card><SectionHeader title="Assignments" text="Accepted opportunities and assignments made directly by MLS." action={viewToggle(view, setView)} /><div className="mt-5">{view === "list" ? <div className="space-y-3">{assignments.map((item) => <AssignmentRow key={item.id} assignment={item} onOpen={actions.openAssignment} />)}{!assignments.length && <EmptyState icon={CalendarDays} title="No assigned work" text="Accepted opportunities and direct assignments will appear here." />}</div> : <AssignmentCalendar assignments={assignments} onOpen={actions.openAssignment} />}</div></Card>
    </div>
  );
}
