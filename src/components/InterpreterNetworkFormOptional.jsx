...
function RadioGroup({ name, options, value, onChange, palette }) {
  return (
    <div className="space-y-3">
      {options.map((option) => (
...
export default function InterpreterNetworkFormOptional({ palette = defaultPalette }) {
  ...
  const getStepErrors = (currentStep) => {
    ...
    if (currentStep === 5) {
      const optionalLinks = [
        ...
        ['certificationsLink', 'certifications / licenses link'],
        ['workSampleLink', 'work sample link'],
      ];

      optionalLinks.forEach(([field, label]) => {
        if (!isValidOptionalUrl(formData[field])) {
          nextErrors[field] = `Enter a valid ${label} beginning with http:// or https://.`;
        }
      });
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-[2rem] border bg-white p-6 shadow-sm md:p-8" style={{ borderColor: p.border }}>
          ...
          <div className="mx-auto mt-6 max-w-3xl space-y-4 text-center text-sm leading-7" style={{ color: p.charcoal }}>
            ...
            <p>
              If available, please share secure links to your resume, credentials, and sample work so they can be reviewed efficiently. You may still submit the form without links.
            </p>
          </div>
          ...
          <SectionCard step={2} totalSteps={totalSteps} borderColor={p.border} titleColor={p.charcoal} title="Credentials & Qualifications" subtitle="Share the core qualification details that help determine fit for future assignments.">
            <div className="space-y-6">
              ...
              <div>
                ...
                <RadioGroup name="stateLicense" options={['Yes', 'No']} value={formData.stateLicense} onChange={(value) => setField('stateLicense', value)} palette={p} />
          ...
          <SectionCard step={4} totalSteps={totalSteps} borderColor={p.border} titleColor={p.charcoal} title="Availability & Preferences" subtitle="Share when and how you are generally available so future requests can be matched more efficiently.">
            <div className="space-y-6">
              ...
              <div>
                ...
                <RadioGroup name="assignmentTypePreference" options={['In-Person', 'VRI', 'Both']} value={formData.assignmentTypePreference} onChange={(value) => setField('assignmentTypePreference', value)} palette={p} />
              ...
              <div>
                ...
                <RadioGroup name="willingToTravel" options={['Yes', 'No']} value={formData.willingToTravel} onChange={(value) => setField('willingToTravel', value)} palette={p} />
              ...
              <div>
                ...
                <div className="mt-4">
                  <RadioGroup name="technicalReadinessConfirmed" options={['Yes', 'No']} value={formData.technicalReadinessConfirmed} onChange={(value) => setField('technicalReadinessConfirmed', value)} palette={p} />
                </div>
        ...
        {step === 5 && (
          <SectionCard step={5} totalSteps={totalSteps} borderColor={p.border} titleColor={p.charcoal} title="Documents & Sample Links" subtitle="Share direct links to your materials if available. You can still submit the form without links.">
            <div className="space-y-6">
              <div>
                <label htmlFor="resumeLink" className={labelClass}>Resume link</label>
                <input id="resumeLink" type="url" value={formData.resumeLink} onChange={(e) => setField('resumeLink', e.target.value)} className={inputClass} placeholder="https://drive.google.com/..., Dropbox, OneDrive, etc." />
              ...
              <div>
                ...
                <input id="certificationsLink" type="url" value={formData.certificationsLink} onChange={(e) => setField('certificationsLink', e.target.value)} className={inputClass} placeholder="https://drive.google.com/..., Dropbox, OneDrive, etc." />
                <ErrorText errors={errors} name="certificationsLink" />
              </div>
              <div>
                <label htmlFor="workSampleLink" className={labelClass}>Work sample link</label>
                ...
                <input id="workSampleLink" type="url" value={formData.workSampleLink} onChange={(e) => setField('workSampleLink', e.target.value)} className={inputClass} placeholder="https://youtube.com/..., Google Drive, Dropbox, OneDrive, etc." />
          ...
          <SectionCard step={6} totalSteps={totalSteps} borderColor={p.border} titleColor={p.charcoal} title="Work Preferences" subtitle="These answers help determine whether a future opportunity is operationally aligned before outreach happens.">
            <div className="space-y-6">
              <div>
                ...
                <RadioGroup name="openToIndependentContractor" options={['Yes', 'No', 'Maybe; I would like more information']} value={formData.openToIndependentContractor} onChange={(value) => setField('openToIndependentContractor', value)} palette={p} />
              ...
              <div>
                ...
                <RadioGroup name="professionalLiabilityInsurance" options={['Yes', 'No', 'Planning to obtain']} value={formData.professionalLiabilityInsurance} onChange={(value) => setField('professionalLiabilityInsurance', value)} palette={p} />
              ...
              <div>
                ...
                <RadioGroup name="comfortableWithStandardIndustryRates" options={['Yes', 'No', 'Would like to discuss']} value={formData.comfortableWithStandardIndustryRates} onChange={(value) => setField('comfortableWithStandardIndustryRates', value)} palette={p} />
        ...
        {step === 7 && (
          <SectionCard step={7} totalSteps={totalSteps} borderColor={p.border} titleColor={p.charcoal} title="Certification & Submission" subtitle="Confirm that the information you provided is accurate before submitting.">
            <div className="space-y-5">
              <label className="flex items-start gap-3 text-sm leading-6" style={{ color: p.charcoal }}>
                <input
                  type="checkbox"
                  checked={formData.certificationAgreement}
                  onChange={(e) => setField('certificationAgreement', e.target.checked)}
                  className="mt-1 h-4 w-4 rounded form-checkbox"
                  style={{ accentColor: p.burgundy }}
                />
                <span>
                  I certify that the information provided is accurate to the best of my knowledge and understand that submitting this form does not guarantee subcontracting opportunities.
                </span>
              </label>
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <button type="button" onClick={handleBack} disabled={step === 1 || isSubmitting} className="btn btn-secondary disabled:cursor-not-allowed disabled:opacity-50">
            Back
          </button>
          {step < totalSteps ? (
